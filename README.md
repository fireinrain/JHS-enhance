...
- 磁力链接高亮匹配已收藏/已下载状态
- DMM 多画质预览视频
- **多源视频预览**：DMMRSC 聚合 API 独立按钮（番号/识别码右侧），HLS 支持、播放进度记忆、画质切换
...
│   │   └── dmmrsc.js            # DMMRSC API（视频多源解析、高清封面获取）
│   ├── lib/                     # 第三方补丁库
...
│       ├── javxy-preview-video.js# DMMRSC 多源视频预览（独立按钮、聚合 API、HLS 支持）
...
- **v3.3.8**
  - 新增：DMMRSC 多源视频预览独立按钮（番号/识别码右侧），基于 DMMRSC 聚合 API
  ...
  - 优化：`dmm.js` 回退至仅 DMM Affiliate API + GraphQL API 双通道
