...
// @require      https://cdn.jsdelivr.net/npm/tabulator-tables@6.5.2/dist/js/tabulator.min.js
...
(function ($$1, localforage$1, Toastify$1, md5$1, Viewer$1, QRCode$1) {
  'use strict';
    ...
    writable: true,
    value: value2
  }) : obj2[key2] = value2)(obj, "symbol" != typeof key ? key + "" : key, value), __privateMethod = (obj, member, method) => (((obj2, member2, msg) => {
    member2.has(obj2) || __typeError("Cannot " + msg);
  })(obj, member, "access private method"), method);
  const currentHref$1 = window.location.href, isJavDb$1 = currentHref$1.includes("javdb"), isJavBus$1 = currentHref$1.includes("javbus") || currentHref$1.includes("seejav") || currentHref$1.includes("bus") || currentHref$1.includes("javsee") || "javbus" === (() => {
    try {
      const _alt = $(".hidden-xs").attr("alt");
      return _alt ? _alt.trim().toLowerCase() : void 0;
    } catch (_e) {
      return void 0;
    }
  })(), isSearchPage = currentHref$1.includes("/search?q") || currentHref$1.includes("/search/") || currentHref$1.includes("/users/"), Status_RUNNING = "RUNNING", Status_SUCCESS = "SUCCESS", Status_FAIL = "FAIL", Status_LOADING = "LOADING", Status_FILTER$1 = "filter", Status_FAVORITE$1 = "favorite", Status_HAS_DOWN$1 = "hasDown", Status_HAS_WATCH$1 = "hasWatch", NO = "no", YES = "yes", qualityOptions = [{
    id: "video-mhb",
    quality: "dmb_w",
...
`;
  function insertStyle(css) {
    if (css) if (css.includes("<style>")) document.head.insertAdjacentHTML("beforeend", css);
    else {
  ...
  isJavBus$1 && insertStyle(javBusStyle);
  isJavDb$1 && insertStyle(javdbStyle);
  insertStyle(
  ...
  window.QRCode = QRCode$1;
  window.currentHref = currentHref$1;
  window.isJavDb = isJavDb$1;
  window.isJavBus = isJavBus$1;
  ...
  window.Status_FILTER = Status_FILTER$1;
  window.Status_FAVORITE = Status_FAVORITE$1;
  ...
  let StorageManager = class _StorageManager {
    ...
    _handleSingleCar(carParam, carList) {
      let { carNum: carNum2, url, names, actionType, publishTime, starId } = carParam;
      if (!carNum2) {
        show.error("番号为空!");
    ...
    async updateCarInfo(carParam) {
      let { carNum: carNum2, url, names, actionType, publishTime, remark } = carParam;
      if (!carNum2) {
        show.error("番号为空!");
        throw new Error("番号为空!");
      }
    ...
    async removeNewVideoList(carNumList) {
      ...
      let hasAnyListBeenModified = false;
    ...
    async addBlacklistItem(item) {
      let { starId, name: name2, allName, role, movieType, url } = item;
    ...
    async batchSaveBlacklistCarList(carDataList) {
      ...
      let blacklistCarList = await this.getBlacklistCarList(), hasChanged = false, copyBlacklistCarList = null, carNumList = [];
      for (const carData of carDataList) {
        ...
        if (existCarData) console.log("已在鉴定记录中", existCarData);
        else {
    ...
    async addFavoriteActressList(actressList) {
      ...
      for (const actress of actressList) {
        let { starId, name: name2, allName, avatar, lastCheckTime, lastPublishTime, actressType } = actress;
        if (!starId) throw new Error("缺失starId");
        if (!name2) throw new Error("缺失name");
        allName || (allName = [name2]);
    ...
    async updateFavoriteActress(actress) {
      const favoriteActressesInfoList = await this.getFavoriteActressList(), { starId, name: name2, allName, avatar, lastCheckTime, newVideoList, lastPublishTime, actressType, remark } = actress;
      if (!starId) throw new Error("缺失starId");
      let favoriteActresses = favoriteActressesInfoList.find(((item) => item.starId === starId));
      if (!favoriteActresses) {
  ...
  class Utils {
    ...
    openPage(url, title, shadeClose, event) {
      ...
      if (event && (event.ctrlKey || event.metaKey)) {
        _GM_openInTab(url.includes("http") ? url : window.location.origin + url, {
          insert: 0
        });
        return;
      }
    ...
    _handleGlobalEscKey(e) {
      ...
      if ($layer.find(".viewer-container").length > 0) viewerExists = true;
      else {
        const iframe = $layer.find(`#layui-layer-iframe${topLayerIndex}`)[0];
        if (iframe && iframe.contentDocument) try {
          $(iframe.contentDocument).find(".viewer-container").length > 0 && (viewerExists = true);
        } catch (error) {
    ...
    loopDetector(condition, after, detectInterval = 20, timeout = 1e4, runWhenTimeout = true) {
      const uuid = Math.random(), start = (/* @__PURE__ */ new Date()).getTime(), stopAndRun = (shouldRun) => {
      ...
      this.intervalContainer[uuid] = setInterval((() => {
        const timeElapsed = (/* @__PURE__ */ new Date()).getTime() - start;
        condition() ? stopAndRun(true) : timeElapsed >= timeout && stopAndRun(runWhenTimeout);
      }), detectInterval);
    }
    ...
    getNowStr(dateSplitStr = "-", timeSplitStr = ":", dateString = null) {
      let now;
      now = dateString ? new Date(dateString) : /* @__PURE__ */ new Date();
    ...
    smoothScrollToTop(duration = 500) {
      return new Promise(((resolve) => {
        ...
        window.requestAnimationFrame((function scrollStep(timestamp) {
          const elapsed = timestamp - start, progress = Math.min(elapsed / duration, 1), easeInOutCubic = progress < 0.5 ? 4 * progress * progress * progress : 1 - Math.pow(-2 * progress + 2, 3) / 2;
    ...
    addCookie(cookieStr, options = {}) {
      const { maxAge = 604800, path = "/", domain = "", secure = false, sameSite = "Lax" } = options;
    ...
    genericSort(arr, sortRules, emptyLast = true) {
  ...
  class GmHttp {
    ...
    postFileFormData(url, data = {}, headers = {}) {
      ...
      let body = "";
      data && Object.keys(data).length > 0 && (body = Object.entries(data).map((([key, value]) => `--${boundary}\r
...
`)).join(""));
    ...
    async downloadFileInChunks(url, headers = {}) {
      ...
      try {
        ...
        if (206 !== sizeResponse.status && 200 !== sizeResponse.status) throw new Error(`请求文件大小失败，状态码: ${sizeResponse.status}`);
        {
          ...
          if (rangeHeader && rangeHeader[1]) fileSize = parseInt(rangeHeader[1], 10);
          else {
          ...
          _GM_xmlhttpRequest({
            ...
            onload: (response) => {
              if (206 === response.status || 200 === response.status) if (response.response instanceof ArrayBuffer) {
                ...
                resolve();
              } else reject(new Error(`第 ${i + 1} 块响应不是 ArrayBuffer。`));
              else reject(new Error(`第 ${i + 1} 块请求失败，状态码: ${response.status}`));
            },
    ...
    async gmRequest(method, url, data = {}, params = {}, headers = {}, noRedirect = false) {
  ...
  class BasePlugin {
    ...
    checkDuplicateCarNumbers(currentList, newList) {
      if (!isJavDb) return false;
      if (!currentList || 0 === currentList.length || !newList || 0 === newList.length) return false;
      const currentCarNums = new Set(currentList.map(((item) => item.carNum)).filter(((n) => n)));
      if (0 === currentCarNums.size) return false;
  ...
  class AutoPagePlugin extends BasePlugin {
    ...
    getInitialPageNumber() {
      if (isJavBus$1) {
        const match = currentHref$1.match(/\/(page|star\/[^/]+)\/(\d+)/);
        return match ? parseInt(match[2], 10) : 1;
      }
      if (isJavDb$1) {
        const match = currentHref$1.match(/[?&]page=(\d+)/);
    ...
    async loadNextPage() {
      ...
      try {
        ...
        let maxPage = 60;
        currentHref$1.includes("c11") && (maxPage = 30);
        if (isJavDb$1 && pageNum > maxPage || currentHref$1.includes("month")) {
          const beyond60Plugin = this.getBean("Beyond60Plugin");
          if (beyond60Plugin) {
            const { html: html2, nextUrl, hasMore } = await beyond60Plugin.handleBeyond60(this.nextUrl);
            if (html2) {
        ...
        isJavBus$1 && $dom.find(".avatar-box").length > 0 && $dom.find(".avatar-box").parent().remove();
    ...
    updatePageUrl(url) {
      ...
      isJavBus$1 && (document.title = document.title.replace(/第\d+頁/, `第${this.currentPage}頁`));
    }
    setState(state, text) {
  ...
  class AliyunApi {
    ...
    async backup(folderName, fileName, uploadContent) {
      if (this.backupFolderId) await this.uploadFile(this.backupFolderId, fileName, uploadContent);
      else {
    ...
    async getBackupList(folderName) {
      let dataList;
      if (this.backupFolderId) dataList = await this.getFileList(this.backupFolderId);
      else {
        await this._createBackupFolder(folderName);
      ...
      return `
            <style>
                ...
                .simple-setting, .mini-simple-setting {
                    ...
                    position: absolute;
                    top: ${isJavDb$1 ? "35px" : "25px"};
                    right: ${isJavDb$1 ? "-300%" : "0"};
                    z-index: 1000;
                    border: 1px solid #ddd;
                    border-radius: 4px;
                ...
                <div style="flex: 1; display: flex; flex-direction: column; height: 100%; ">
                    <div style="flex: 1; margin: 0 10px; padding-bottom: 20px;overflow: hidden">
                        ...
                        <div id="base-panel" class="content-panel" style="display: ${"base-panel" === defaultActivePanel ? "block" : "none"};">
                            ...
                            <div class="setting-item ${isJavDb$1 ? "" : "do-hide"}">
                                <span class="setting-label">
                        ...
                        <div id="task-panel" class="content-panel" style="display: ${"task-panel" === defaultActivePanel ? "block" : "none"};">
                            ...
                            <div id="setting-checkFavoriteActress" style="border: 1px solid #ccc; padding: 10px; margin-bottom: 15px;" class="${isJavDb$1 ? "" : "do-hide"}">
                            ...
                            <div id="setting-checkNewVideo" style="border: 1px solid #ccc; padding: 10px; margin-bottom: 15px;" class="${isJavDb$1 ? "" : "do-hide"}">
...
`,
    ...
    addLabelTag(containerId, keyword) {
      ...
      let $label, color = "#333";
      if (/^[a-z]{2,}-/i.test(keyword) && isJavDb$1) {
        color = "#3477ad";
        $label = $(`
  ...
  class TaskPlugin extends BasePlugin {
    ...
    isUnnecessaryCheck(lastCheckTimeStr, checkIntervalTime) {
      ...
      return utils.getHourDifference(new Date(lastCheckTimeStr), /* @__PURE__ */ new Date()) < checkIntervalTime;
    }
    handle() {
      this.doTask().then();
    }
    ...
    async checkBlacklist(isManual) {
      ...
      for (const blacklistItem of blacklist) {
        ...
        if (new URL(window.location.href).hostname === new URL(url).hostname) {
          if (isManual || !checkTime || !this.isUnnecessaryCheck(checkTime, checkBlacklist_intervalTime)) if (!lastPublishTime || 0 === checkBlacklist_ruleTime || this.isUnnecessaryCheck(lastPublishTime, checkBlacklist_ruleTime)) taskParamList.push(blacklistItem);
          else {
      ...
      await this.limitConcurrency(taskParamList, checkConcurrencyCount, checkRequestSleep, (async (task) => {
        let { starId, name: name2, url } = task;
        try {
          ...
          this.storageQueue.addTask((async () => {
            let { lastPublishTime } = await blacklistPlugin.parseAndSaveFilterInfo($dom, name2, starId);
    ...
    async checkNewVideo(isManual) {
      ...
      for (const actress of actressInfoList) {
        const { lastCheckTime, lastPublishTime, name: name2 } = actress;
      ...
      await this.limitConcurrency(taskParamList, checkConcurrencyCount, checkRequestSleep, (async (task) => {
        const { lastCheckTime, name: name2, starId } = task;
    ...
    async parsePage($dom, starId, name2, filterKeywordList, filterActorActressCarNumList) {
      let movieList, nextPageLink, tempIsJavBus = false, selectorType = "javdb";
      ...
      for (const element of movieList) {
        const $item = $(element), { carNum: carNum2, url, title, publishTime } = this.getBoxCarInfo($item);
        if (!carNum2) continue;
    ...
    async checkOneNewVideo(actress) {
      const filterKeywordList = await storageManager.getTitleFilterKeyword(), filterActorActressCarList = await storageManager.getBlacklistCarList(), filterActorActressCarNumList = new Set(filterActorActressCarList.map(((car) => car.carNum))), { lastCheckTime, name: name2, starId } = actress;
      let url = `${this.javDbUrl}/actors/${starId}?t=d`;
  ...
  let currentCdnIndex$1 = parseInt(localStorage.getItem("jhs_img_cdn_index") || "0", 10);
  (currentCdnIndex$1 >= CDN_SOURCES.length || currentCdnIndex$1 < 0) && (currentCdnIndex$1 = 0);
  window.G_FRIENDS_JSON_URL = CDN_SOURCES[currentCdnIndex$1].json;
  window.window.CDN_BASE_URL = CDN_SOURCES[currentCdnIndex$1].base;
  ...
  class NewVideoPlugin extends BasePlugin {
      ...
      $("#checkNewVideo").on("click", ((event) => {
        utils.q({
          ...
          navigator.locks.request(taskPlugin.singleTaskKey, {
            ...
            lock ? await taskPlugin.checkNewVideo(true) : show.error("当前有定时任务在后台执行中, 无法发起手动任务");
          })).catch(((error) => {
            console.error("锁任务出现错误:", error);
            clog.error("锁任务出现错误:", error);
          }));
        ...
        success: (layero, index) => {
          ...
          $("#select-cdn-btn").on("click", (async () => {
            await (async function() {
              const initialIndex = currentCdnIndex, radioOptions = CDN_SOURCES.map(((source, index2) => `
        <div style="margin-bottom: 10px;">
            <input type="radio" id="cdn-${index2}" name="cdn-source" value="${index2}" ${index2 === initialIndex ? "checked" : ""} style="margin-right: 10px;">
            <label for="cdn-${index2}">${source.name} ${source.json.includes("jsdelivr") ? "(推荐)" : ""}</label>
        </div>
    `)).join(""), cdnSelectHtml = `
        <div style="padding: 20px;">
    ...
    `;
              layer.open({
                ...
                btn: ["确定", "取消"],
                success: (layero2, index2) => {
                  utils.setupEscClose(index2);
                },
                yes: async (index2) => {
        ...
        yes: async (index) => {
          ...
          if (await storageManager.updateFavoriteActress(actress)) show.error("修改失败");
          else {
  ...
  class HistoryPlugin extends BasePlugin {
    ...
    handle() {
      ...
      isJavBus$1 && utils.loopDetector((() => $("#setting-btn").length), (() => {
    ...
    bindClick() {
      ...
      $(document).on("click", ".history-deleteBtn, .history-filterBtn, .history-favoriteBtn, .history-hasDownBtn, .history-hasWatchBtn, .history-detailBtn", ((event) => {
        ...
        $btn.hasClass("history-filterBtn") ? utils.q(event, `是否屏蔽${carNum2}?`, (() => handleAction(Status_FILTER$1))) : $btn.hasClass("history-favoriteBtn") ? handleAction(Status_FAVORITE$1).then() : $btn.hasClass("history-hasDownBtn") ? handleAction(Status_HAS_DOWN$1).then() : $btn.hasClass("history-hasWatchBtn") ? handleAction(Status_HAS_WATCH$1).then() : $btn.hasClass("history-deleteBtn") ? this.handleDelete(event, carNum2) : $btn.hasClass("history-detailBtn") && this.handleClickDetail(event, {
          carNum: carNum2,
          url: aHref
        }).then();
      }));
        ...
        utils.q(event, `当前已勾选${selectedRows.length}条数据, 是否全标记为 ${handleText}?`, (async () => {
          ...
          try {
            if ("delete" === handleStatus) {
              ...
              removedCount > 0 ? show.ok(`已成功删除 ${removedCount} 个番号`) : false === removedCount && show.error("提供的番号中没有一个存在于列表中。");
            } else {
    ...
    async getDataList(page, size, sort) {
      ...
      dataList.forEach(((item) => {
        switch (item.status) {
          case Status_FILTER$1:
            this.filterCount++;
            break;
          case Status_FAVORITE$1:
            this.favoriteCount++;
            break;
          case Status_HAS_DOWN$1:
            this.hasDownCount++;
            break;
          case Status_HAS_WATCH$1:
            this.hasWatchCount++;
        }
      }));
    ...
    async handleClickDetail(event, data) {
      if (isJavDb$1) if (data.carNum.includes("FC2-")) {
        ...
        utils.openPage(data.url, data.carNum, false, event);
      }
      if (isJavBus$1) {
        let url = data.url;
        ...
        else utils.openPage(data.url, data.carNum, false, event);
      }
    }
            ...
            <div style="padding: 20px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <select id="edit-status" style="width: 100%; padding: 10px; border: 1px solid #ddd;">
                        ...
                        ${[{
      value: Status_FILTER$1,
      text: "🚫 屏蔽"
    }, {
      value: Status_FAVORITE$1,
      text: "⭐ 收藏"
    }, {
      value: Status_HAS_DOWN$1,
      text: "📥️ 已下载"
    }, {
      value: Status_HAS_WATCH$1,
      text: "🔍 已观看"
    }].map(((option) => `
  ...
  const _unsafeWindow$1 = /* @__PURE__ */ (() => typeof unsafeWindow !== "undefined" ? unsafeWindow : window)();
    ...
    handleFancyBox() {
      document.addEventListener("click", (function(e) {
        if (e.target.closest(".fancybox-button--thumbs")) {
          ...
          _unsafeWindow$1.$.fancybox.defaults.thumbs.autoStart = isVisible;
        }
      }));
      if (void 0 !== _unsafeWindow$1.$.fancybox) {
        const savedState = localStorage.getItem("jhs_fancyboxThumbs");
        _unsafeWindow$1.$.fancybox.defaults.thumbs.autoStart = "true" === savedState;
      }
    }
  }
  class PreviewVideoPlugin extends BasePlugin {
    ...
    async initDmm() {
      try {
        const dmmVideoMap = await getDmmVideo(this.getPageInfo().carNum, false);
  ...
  const _HotkeyManager = class _HotkeyManager2 {
    constructor() {
      if (new.target === _HotkeyManager2) throw new Error("HotkeyManager cannot be instantiated.");
    }
  ...
  class SubTitleCatPlugin extends BasePlugin {
    ...
    window.JavPackSubtitle = (_a = class {
        ...
        static buildSubtitleBaseName(details = {}) {
            var _a2;
            if ((_a2 = window.JavPackMatch115Console) == null ? void 0 : _a2.buildRename) {
        ...
        static getTargetCid() {
            var _a2, _b;
            return ((_a2 = document.querySelector(".x-match-cont .zymatch-item [data-cid]")) == null ? void 0 : _a2.dataset.cid) || ((_b = document.querySelector(".x-match-cont .zymatch-item")) == null ? void 0 : _b.dataset.cid) || "";
        }
    ...
    `;
        ...
        static openSearchModal({details = {}, getTargetCid} = {}) {
            ...
            (_a2 = document.getElementById(this.MODAL_ID)) == null ? void 0 : _a2.remove();
        ...
        static parseSubtitleCatSearch(html) {
            ...
            return [...doc.querySelectorAll("table.sub-table tbody tr")].flatMap((row) => {
                ...
                const name2 = (_a2 = link == null ? void 0 : link.textContent) == null ? void 0 : _a2.trim();
        ...
        static parseSubtitleCatLanguages(html, entry) {
            ...
            return [...doc.querySelectorAll(".sub-single")].flatMap((node) => {
                var _a2;
                const languageCode = ((_a2 = node.querySelector("img.flag")) == null ? void 0 : _a2.getAttribute("alt")) || "";
    ...
    `;
        ...
        static renderRow(item, index, highlightRegex) {
            var _a2;
            const lang = ((_a2 = item.languages) == null ? void 0 : _a2[0]) || "未知";
        ...
        static async handleAction({btn, item, previewBox, statusNode, overlay, details, getTargetCid}) {
            ...
            try {
                ...
                if (action === "preview") {
                    ...
                    const cid = (getTargetCid == null ? void 0 : getTargetCid()) || this.getTargetCid();
                    ...
                    (_b = (_a2 = this.getGrant()) == null ? void 0 : _a2.notify) == null ? void 0 : _b.call(_a2, {
                        icon: "success",
                        msg: "字幕已上传到 115"
                    });
                    (_c = document.getElementById(this.BTN_ID)) == null ? void 0 : _c.classList.add("is-success");
    ...
    }, __publicField(_a, "BTN_ID", "x-subtitle-search-btn"), __publicField(_a, "MODAL_ID", "x-subtitle-modal"), __publicField(_a, "previewCache", /* @__PURE__ */ new Map()), __publicField(_a, "SUBTITLECAT_ORIGIN", "https://www.subtitlecat.com"), __publicField(_a, "SUBTITLECAT_CHINESE_LANGUAGES", /* @__PURE__ */ new Map([
        ["zh-CN", "简中"],
        ["zh-TW", "繁中"]
    ])), _a);
  class Fc2Plugin extends BasePlugin {
    ...
    handle() {
      ...
      if (currentHref$1.includes("advanced_search?type=3")) {
        $("h2.section-title").contents().first().replaceWith("Fc2PPV");
        $(".section .container > .box").remove();
      }
      if (currentHref$1.includes("collection_codes?movieId")) {
        $("section").html("");
    ...
    handleMovieDetail(movieId) {
      javDbApi.getMovieDetail(movieId).then(((res) => {
        ...
        $(".movie-info-container").html(`
            ...
            `);
        this.getBean("TranslatePlugin").translate(res.carNum, false).then();
      })).catch(((err) => {
        console.error(err);
        $(".movie-info-container").html(`
  ...
  class HighlightMagnetPlugin extends BasePlugin {
    ...
    showAll() {
      ...
      isJavBus$1 && $("#magnet-table tr").toArray().forEach(((el) => $(el).show()));
    }
  }
  class FoldCategoryPlugin extends BasePlugin {
    ...
    async handle() {
      if (window.isListPage && !currentHref$1.includes("advanced_search")) {
        this.highlightTag();
        utils.loopDetector((() => $("#waitCheckBtn").length), (() => {
          this.createFoldBtn();
        }), 1, 1e4, true);
...
${err.stack}` : "");
  ...
  class TOP250Plugin extends BasePlugin {
    ...
    renderPagination() {
      ...
      this.$contentBox.on("click", ".pagination-link, .pagination-previous, .pagination-next", ((event) => {
        ...
        !isNaN(newPage) && newPage > 0 && ((newPage2) => {
          urlParams.set("page", newPage2);
    ...
    toolBar(type, type_value, currentPage) {
      ...
      let yearHtml = "";
      for (let year = (/* @__PURE__ */ new Date()).getFullYear(); year >= 2008; year--) yearHtml += `
    ...
    async checkLogin(event, urlParams) {
      ...
      event && (event.ctrlKey || event.metaKey) ? _GM_openInTab(window.location.origin + url, {
        insert: 0
  ...
  class NavBarPlugin extends BasePlugin {
    ...
    hookOldSearch() {
      ...
      const clonedImage = searchImage.cloneNode(true);
  ...
  class BusPreviewVideoPlugin extends BasePlugin {
    ...
    async handle() {
      ...
      "yes" === await storageManager.getSetting("enableLoadPreviewVideo", "yes") && getDmmVideo(this.getPageInfo().carNum, false).then();
      let isHandlingVideo = false, $preview = $(".preview-video-container");
      $preview.on("click", (async (event) => {
        event.preventDefault();
        event.stopPropagation();
        if (isHandlingVideo) show.info("正在加载中, 勿重复点击");
        else {
          ...
          if (!this.isUploading) {
            ...
            try {
              ...
              this.siteList.forEach(((site) => {
                const siteUrl = site.url.replace("{占位符}", encodeURIComponent(imgUrl)), isChecked = false !== selectedSites[site.name];
      ...
      if (dataList) if (0 !== dataList.length) {
        ...
        if (20 === dataList.length) {
          ...
          $loadMoreRelateds.on("click", (async () => {
            $loadMoreRelateds.text("加载中...").prop("disabled", true);
            currentPage++;
            let moreData;
            try {
              ...
              $loadMoreRelateds.text("加载失败, 请点击重试").prop("disabled", false);
            }
            if (moreData) {
              ...
              if (moreData.length < 20) {
                ...
                $("#relatedEnd").show();
              } else $loadMoreRelateds.text("加载更多清单").prop("disabled", false);
            }
          }));
        } else $relatedFooter.html('<div style="text-align:center; padding:10px; color:#666; margin-top:10px;">已加载全部清单</div>');
      } else $relatedContainer.append('<div style="margin-top:15px;background-color:#ffffff;padding:10px;margin-left: -10px;">无清单</div>');
      else {
  ...
  class MagnetHubPlugin extends BasePlugin {
    ...
    searchEngine($container, engine, keyword) {
      ...
      engine.parseHtml && _GM_xmlhttpRequest({
        method: "GET",
        url,
        onload: (response) => {
          try {
  ...
  class WangPan115TaskPlugin extends BasePlugin {
    ...
    async getSavePathId(nyName) {
      ...
      savePath115 = savePath115.replaceAll("{date}", utils.formatDate(/* @__PURE__ */ new Date()));
      return savePath115;
    }
    async handleAddTask(magnetLink, savePath) {
      ...
      const sign = singInfo.sign, time = singInfo.time, userId = this.getUserId(), result = await (async (magnet, wp_path_id = "", uid, sign2, time2) => {
        const data = {
      ...
      false === result.state && (title = result.error_msg + " 是否前往查看?");
    ...
    async getFileId(userId, sign, time, infoHash) {
      const taskList = await (async (uid, sign2, time2) => {
        const data = {
          page: 1,
          uid,
          sign: sign2,
  ...
  class ScreenShotPlugin extends BasePlugin {
    ...
    async loadScreenShot() {
      ...
      let carNum2 = this.getPageInfo().carNum;
      isJavDb$1 && $(".preview-images .tile-item").first().before(' <a class="tile-item screen-container" style="overflow:hidden;max-height: 215px;text-align:center;"><div style="margin-top: 50px;color: #000;cursor: auto">正在加载缩略图</div></a> ');
      isJavBus$1 && $("#sample-waterfall .sample-box:first").after(' <a class="sample-box screen-container" style="overflow:hidden; height: 110px; text-align:center;"><div style="margin-top: 30px;color: #000;cursor: auto">正在加载缩略图</div></a> ');
      try {
    ...
    addImg(title, imgUrl) {
      if (imgUrl) {
        isJavDb$1 && $(".screen-container").html(`<img src="${imgUrl}" alt="${title}" loading="lazy" style="width: 100%;">`);
        isJavBus$1 && $(".screen-container").html(`<div class="photo-frame"><img src="${imgUrl}" style="height: inherit;width: 100%;" title="${title}" alt="${title}"></div>`);
  ...
  class WangPan115Plugin extends BasePlugin {
    ...
    async handle() {
      if (!currentHref$1.includes("&ac=userfile") && currentHref$1.includes("115")) {
    ...
    bindClick() {
      ...
      $("#login-115-type").on("change", (async (event) => {
        ...
        const checkLoginRecursive = async () => {
          try {
            const loginResult = await (async (uid2, time2, sign2) => {
              let url = `https://qrcodeapi.115.com/get/status/?uid=${uid2}&time=${time2}&sign=${sign2}`;
              return await gmHttp.get(url);
            })(uid, time, sign);
            ...
            if (2 === status) {
              show.ok("扫码登录成功");
              const checkResult = await (async (loginType, uid2) => {
                const data2 = {
                  app: loginType,
                  account: uid2
                }, url = `https://passportapi.115.com/app/1.0/${loginType}/1.0/login/qrcode/`;
                return await gmHttp.postFileFormData(url, data2);
              })(login115Type, uid);
  ...
  const _WangPan115MatchPlugin = class _WangPan115MatchPlugin2 extends BasePlugin {
    ...
    async matchDetailPage(matchList) {
      ...
      if (isJavDb$1) if ($("#all-match-box").length) $("#all-match-box").append($detail);
      else {
    ...
    async checkLoginStatus() {
      var _a2;
      if (this.loginStatus === _WangPan115MatchPlugin2.LoginStatus.UNCHECKED) try {
        const testResult = await searchFiles("test");
        this.loginStatus = (null == (_a2 = testResult.error) ? void 0 : _a2.includes("登录")) ? _WangPan115MatchPlugin2.LoginStatus.LOGGED_OUT : _WangPan115MatchPlugin2.LoginStatus.LOGGED_IN;
      } catch {
        this.loginStatus = _WangPan115MatchPlugin2.LoginStatus.LOGGED_OUT;
      }
    }
    async processMovieElements(movieListElement) {
      const promises = Array.from(movieListElement).filter(((ele) => !utils.isHidden(ele))).filter(((ele) => !(isJavBus$1 && $(ele).find(".avatar-box").length > 0))).map(((ele) => this.processSingleMovieElement(ele)));
      await Promise.all(promises);
    }
    async processSingleMovieElement(element) {
      const $box2 = $(element), { carNum: carNum2 } = this.getBoxCarInfo($box2);
      if (!($box2.find("[class^='jhs-match-']").length > 0)) if (this.loginStatus !== _WangPan115MatchPlugin2.LoginStatus.LOGGED_OUT) try {
        const matchList = await this.searchFiles(carNum2);
    ...
    addTag($box2, carNum2, matchList) {
      if (!($box2.find("[class^='jhs-match-']").length > 0)) if (this.loginStatus === _WangPan115MatchPlugin2.LoginStatus.LOGGED_OUT) $box2.find(".video-title").prepend(`<a class='jhs-match-no-login-btn a-info' 
                   data-keyword="${carNum2}" 
                   title="未登录115网盘">未登录</a>`);
      else if (matchList.length > 0) {
  ...
  class ListPageButtonPlugin extends BasePlugin {
    ...
    async createMenuBtn() {
      ...
      if (isJavDb$1) {
        ...
        } else currentHref$1.includes("/tags") && utils.loopDetector((() => $("#jhs-check-tag").text().trim()), (async () => {
        ...
        const isFc2Page2 = currentHref$1.includes("advanced_search");
      ...
      if (isJavBus$1) {
        const isStarPage = currentHref$1.includes("/star/");
    ...
    bindEvent() {
      ...
      $("#filterAllVideo").on("click", (async (event) => {
        ...
        }, $actor = isJavDb$1 ? $(".actor-section-name") : $(".avatar-box .photo-info .pb10");
        if (0 === $actor.length) {
          show.error("获取演员名称失败");
          return;
        }
    ...
    async openWaitCheck(event) {
      ...
      $(`${this.getSelector().itemSelector}:visible`).each(((i, el) => {
        ...
        const { carNum: carNum2, aHref } = this.getBoxCarInfo($box2);
  ...
  class OtherSitePlugin extends BasePlugin {
    ...
    async loadOtherSite(carNum2, sourceCarNum) {
      ...
      const enabledSites = this.getEnabledSites(), html = `
            <div id="otherSiteBox" class="panel-block" style="${isJavDb$1 ? "margin-top:8px;font-size:13px" : "margin-top:10px;font-size:13px"}; user-select: none; ">
      ...
      $("#javTrailersBtn").on("click", (async (event) => {
        ...
        utils.openPage(url, carNum2, false, event);
      }));
      await Promise.all(this.siteConfigs.map((async (config) => {
        config.condition && false === config.condition(config.sourceCarNum) || await this.handleSite(carNum2, config);
      })));
    ...
    async handleSite(carNum2, config) {
      ...
      if (config.noHandle && true === config.noHandle) {
        ...
        const html = await gmHttp.get(searchUrl, null, config.headers, true), $dom = utils.htmlTo$dom(html), resultUrlList = [];
    ...
    renderSettingsArea() {
      ...
      checkboxesDiv && (checkboxesDiv.innerHTML = this.siteConfigs.map(((config) => {
        ...
        return `
                <div style="margin-right: 15px; display: flex; align-items: ${isJavDb$1 ? "center" : "flex-start"};">
    ...
    async fetchAndDisplayReviews(movieId) {
      ...
      if (dataList.length === reviewCount) {
        ...
        $loadMoreReviews.on("click", (async () => {
          $loadMoreReviews.text("加载中...").prop("disabled", true);
          currentPage++;
          let moreData;
          try {
            ...
            $loadMoreReviews.text("加载失败, 请点击重试").prop("disabled", false);
          }
          if (moreData) {
            ...
            if (moreData.length < reviewCount) {
              ...
              $("#reviewsEnd").show();
            } else $loadMoreReviews.text("加载更多评论").prop("disabled", false);
          }
        }));
  ...
  class CoverButtonPlugin extends BasePlugin {
    ...
    async initCss() {
      return `
            <style>
                ...
                ${isJavBus$1 ? ".tool-box .icon, .setting-label .icon{ height: 24px; width: 24px; }" : ""}
                .tool-box svg path {
                  fill: blue;
                }
    ...
    async enableSvgBtn() {
      const settingObj = await storageManager.getSetting(), { enableScreenSvg = YES, enableVideoSvg = YES, enableHandleSvg = YES, enableSiteSvg = YES, enableCopySvg = YES } = settingObj;
      [{
        selector: ".screenSvg",
    ...
    async bindClick() {
      ...
      $(document).on("click", ".more-tools-container", ((event) => {
        ...
        $(".more-tools").not($currentTools).stop(true, true).removeClass("elastic-in").addClass("elastic-out").hide();
        $currentTools.is(":visible") ? $currentTools.stop(true, true).removeClass("elastic-in").addClass("elastic-out").hide() : $currentTools.stop(true, true).removeClass("elastic-out").addClass("elastic-in").show();
      }));
      $(document).on("click", (function(event) {
        $(event.target).closest(".more-tools-container").length || $(".more-tools").stop(true, true).removeClass("elastic-in").addClass("elastic-out").hide();
      }));
      ...
      $(document).on("click", ".filterBtn, .favoriteBtn, .hasDownBtn, .hasWatchBtn", ((event) => {
        event.preventDefault();
        event.stopPropagation();
        const $btn = $(event.target).closest(".menu-btn"), $box2 = $btn.closest(".item"), { carNum: carNum2, url, publishTime } = this.getBoxCarInfo($box2), handleAction = async (status) => {
  ...
  class Fc2By123AvPlugin extends BasePlugin {
    ...
    handle() {
      ...
      if (currentHref$1.includes("/advanced_search?type=100")) {
        this.hookPage();
    ...
    async loadData(carNum2, href) {
      let loadObj = loading();
      try {
        const { id, publishDate, title, moviePoster } = await this.get123AvVideoInfo(href);
        $(".movie-info-container").html(`
        ...
        this.getBean("TranslatePlugin").translate(carNum2, false).then();
      } catch (e) {
        console.error(e);
      } finally {
        loadObj.close();
  ...
  class DetailPageButtonPlugin extends BasePlugin {
    ...
    speedVideo() {
      if ($("#preview-video").is(":visible")) {
        ...
        if (videoEl) {
          ...
          if (videoEl.currentTime + 5 < videoEl.duration) videoEl.currentTime += 5;
          else {
    ...
    async bindHotkey() {
      ...
      this.filterHotKey && (handlers[this.filterHotKey] = () => {
        this.answerCount >= 2 ? this.filterOne(null, true) : this.filterOne(null);
        this.answerCount++;
      });
      ...
      const registerHotkey = (key, handler) => {
        HotkeyManager.registerHotkey(key, ((event) => {
          ...
          "INPUT" === activeElement.tagName || "TEXTAREA" === activeElement.tagName || activeElement.isContentEditable || (window.isDetailPage ? handler() : ((message) => {
            ...
            if (0 === childIframe.length) return false;
...
`;
  ...
  class BlacklistPlugin extends BasePlugin {
    ...
    async addBlacklist(event) {
      ...
      let dataInfo, title;
      if (currentHref$1.includes("/tags")) {
        const urlObj = new URL(currentHref$1);
      ...
      const { starId, name: name2, allName, role, movieType, blacklistUrl } = dataInfo;
      currentHref$1.includes("page") && !currentHref$1.includes("page=1") && (title += "<br/> 注意: 当前页面非第一页, 屏蔽数据将从此页面开始");
      if (isJavBus$1) {
        const afterStarParts = currentHref$1.split("/star/")[1].split("/");
            ...
            <div style="padding: 10px 20px; height: 100%;overflow:hidden;"> 
                 <div style="display: flex;justify-content: space-between;">
                    ...
                    <div style="display: flex; gap:5px">
                        ...
                        <select id="urlType" data-tip="在演员页屏蔽时,是否选择了分类" style="text-align: center;min-width: 150px; ${isJavDb$1 ? "" : "display: none;"}">
      ...
      layer.open({
        ...
        success: async (layero) => {
          ...
          $(".layui-layer-content").on("click", "#cleanQueryBtn", (async (event) => {
            ...
            utils.openPage(url, name2, true, event);
            ...
            utils.q({
              ...
              navigator.locks.request(taskPlugin.singleTaskKey, {
                ...
                if (lock) {
                  ...
                  await taskPlugin.checkBlacklist(true);
    ...
    async getTableData() {
      ...
      const carListMap = /* @__PURE__ */ new Map();
    ...
    async filterAllVideo(actorName, $dom) {
      ...
      for (const element of movieList) {
        const $item = $(element), { carNum: carNum2, url, publishTime } = this.getBoxCarInfo($item);
        if (url && carNum2) try {
    ...
    async filterActorVideo(actorName, starId, $dom) {
      let { nextPageLink } = await this.parseAndSaveFilterInfo($dom, actorName, starId);
      ...
      if (nextPageLink) {
        ...
        if (isJavDb$1 && beyond60Plugin && pageNum > 60) {
          let { html, nextUrl, hasMore } = await beyond60Plugin.handleBeyond60(nextPageLink), mergeHtml = `
    ...
    async parseAndSaveFilterInfo($dom, actorName, starId) {
      let movieList, nextPageLink;
      if ($dom) {
        let tempIsJavBus = false, selectorType = "javdb";
        if ($dom.text().includes("javbus")) {
          tempIsJavBus = true;
          selectorType = "javbus";
        }
      ...
      for (const element of movieList) {
        const $item = $(element), { carNum: carNum2, url, publishTime, title } = this.getBoxCarInfo($item);
        lastPublishTime || (lastPublishTime = publishTime);
    ...
    async clearKeywordBlacklist() {
      ...
      const processedKeywordsSet = /* @__PURE__ */ new Set();
      ...
      0 !== carNumCount ? layer.open({
        ...
        yes: async (index) => {
          ...
          if (0 === removedCount) show.error("移除失败");
          else {
  ...
  class ListPagePlugin extends BasePlugin {
    ...
    async doFilter() {
      ...
      if (movieList.length) {
        ...
        await this.getBean("WangPan115MatchPlugin").matchMovieList(movieList);
        isJavBus$1 && await this.getBean("BusImgPlugin").logImageHeightsByRow();
      }
    }
    async filterMovieList(movieList) {
      ...
      const [carList, filterKeywordList, blacklist, blacklistCarList, settingObj] = await Promise.all([storageManager.getCarList(), storageManager.getTitleFilterKeyword(), storageManager.getBlacklist(), storageManager.getBlacklistCarList(), storageManager.getSetting()]), t1 = utils.time("读取数据耗时"), carNumSets = carList.reduce(((dataMap, item) => {
        ...
        [Status_FILTER$1]: /* @__PURE__ */ new Set(),
        [Status_FAVORITE$1]: /* @__PURE__ */ new Set(),
        [Status_HAS_DOWN$1]: /* @__PURE__ */ new Set(),
        [Status_HAS_WATCH$1]: /* @__PURE__ */ new Set()
      });
      utils.time("组装数据耗时");
      const starIdToRoleMap = new Map(blacklist.map(((item) => [item.starId, item.role]))), { actorCarNumToNameMap, actressCarNumToNameMap } = blacklistCarList.reduce(((dataMap, carItem) => {
      ...
      await Promise.all(movieList.map((async (ele) => {
        ...
        if (tag.text) {
          const tagHtml = isJavDb$1 ? `<span class="tag is-success status-tag" data-tip="${filterReason}" title=""
          ...
          isJavDb$1 && $box2.find(".tags").append(tagHtml);
          if (isJavBus$1) {
    ...
    async bindClick() {
      ...
      $(selector.boxSelector).on("click", ".item img", (async (event) => {
        ...
        const $box2 = $(event.target).closest(".item"), { carNum: carNum2, aHref } = this.getBoxCarInfo($box2);
      ...
      $(selector.boxSelector).on("click", ".item .video-title", (async (event) => {
        ...
        const $box2 = $(event.currentTarget).closest(".item"), { carNum: carNum2, aHref } = this.getBoxCarInfo($box2);
        if (carNum2.includes("FC2-")) {
          event.preventDefault();
          let movieId = this.parseMovieId(aHref);
          this.getBean("Fc2Plugin").openFc2Dialog(movieId, carNum2, aHref);
      ...
      $(selector.boxSelector).on("contextmenu", ".item img, .item video", (async (event) => {
        event.preventDefault();
        const $box2 = $(event.target).closest(".item"), { carNum: carNum2, url, publishTime } = this.getBoxCarInfo($box2);
        let $actor = isJavDb$1 ? $(".actor-section-name") : $(".avatar-box .photo-info .pb10"), actorName = "";
    ...
    async parseActressName(url) {
      ...
      if (await storageManager.getSetting("enableSaveActressCarInfo", NO) === YES) {
        ...
        const html = await gmHttp.get(url), $dom = utils.htmlTo$dom(html);
        isJavDb$1 ? actorName = $dom.find(".female").prev().map(((i, el) => $(el).text())).get().join(" ") : isJavBus$1 && (actorName = $dom.find('span[onmouseover*="star_"] a').map(((i, el) => $(el).text())).get().join(" "));
    ...
    replaceHdImg(coverImgNodeList) {
      ...
      isJavDb$1 && coverImgNodeList.forEach(((img) => {
    ...
    addJumpPageControl() {
      ...
      const currentPageNum = utils.getUrlParam(currentHref$1, "page") || 1, $input = $("<input>", {
        type: "number",
        id: "jumpPageInput",
        placeholder: "页码",
        min: "1",
  ...
  const _unsafeWindow = /* @__PURE__ */ (() => typeof unsafeWindow !== "undefined" ? unsafeWindow : window)();
  ...
  !(function() {
    const showMessage = (msg, type, gravityOrOptions, positionOrOptions, options) => {
      ...
      -1 === defaultConfig.duration && (defaultConfig.close = true);
  ...
  !(function() {
    ...
    window.showImageViewer = function(imgUrlOrContainer, altText = "") {
      let $container = null, isTempContainer = false;
  ...
  window.ImageHoverPreview = class {
    ...
    handleMouseMove(e) {
      ...
      let { offsetX, offsetY } = this.config, left = e.clientX + offsetX, top = e.clientY + offsetY;
  ...
  !(async function() {
    ...
    class CLog {
      ...
      tryInitialize() {
        if ("loading" === document.readyState) return false;
      ...
      addLog(primaryContent, type = "base", ...optionalParams) {
        ...
        allContents.forEach(((content) => {
          if ("[object Error]" === Object.prototype.toString.call(content)) processedParts.push(String(content));
          else if ("object" == typeof content && null !== content) try {
  ...
  !(function() {
    ...
    function positionTooltip(target, content, preferredPosition) {
      const el = (function(content2) {
        const el2 = document.createElement("div");
        el2.classList.add("js-tooltip");
        const contentEl = document.createElement("div");
        contentEl.innerHTML = content2;
  ...
  !(async function() {
    window.isDetailPage = (function() {
      let href = window.location.href;
      return isJavDb$1 ? href.split("?")[0].includes("/v/") : !!isJavBus$1 && $("#magnet-table").length > 0;
    })();
    window.isListPage = (function() {
      let href = window.location.href;
      return isJavDb$1 ? $(".movie-list").length > 0 || href.includes("advanced_search") : !!isJavBus$1 && $(".masonry > div .item").length > 0;
    })();
    window.isFc2Page = (function() {
      let href = window.location.href;
      return href.includes("advanced_search?type=3") || href.includes("advanced_search?type=100");
    })();
    isJavDb$1 && /(^|;)\s*locale\s*=\s*en\s*($|;)/i.test(document.cookie) && show.error("请切换到中文语言下才可正常使用本脚本", {
      duration: -1
    });
