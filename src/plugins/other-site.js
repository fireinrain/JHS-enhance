...
class OtherSitePlugin extends BasePlugin {
    ...
    async loadOtherSite(carNum2, sourceCarNum) {
        ...
        $("#javTrailersBtn").on("click", (async event => {
            ...
            const filterHotKey = settingObj.filterHotKey, favoriteHotKey = settingObj.favoriteHotKey,
                speedVideoHotKey = settingObj.speedVideoHotKey;
            let href = $("#javTrailersBtn").attr("href"),
                url = href + `?handle=1&filterHotKey=${filterHotKey}&favoriteHotKey=${favoriteHotKey}&speedVideoHotKey=${speedVideoHotKey}`;
    ...
    async handleSite(carNum2, config) {
        ...
        try {
            ...
            const cacheKey = "jhs_other_site",
                cacheData = localStorage.getItem(cacheKey) ? JSON.parse(localStorage.getItem(cacheKey)) : {},
                siteKey = carNum2 + "_" + config.id.replace("Btn", ""), cachedResult = cacheData[siteKey];
            if (cachedResult) {
            ...
            const html = await gmHttp.get(searchUrl, null, config.headers, !0), $dom = utils.htmlTo$dom(html),
                resultUrlList = [];
