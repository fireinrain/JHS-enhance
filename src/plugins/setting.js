...
class SettingPlugin extends BasePlugin {
    ...
    async initSimpleSettingForm() {
        ...
        $("#enable115AutoRename").prop("checked", !!settingObj.enable115AutoRename && settingObj.enable115AutoRename === YES);
        $("#autoRenameScope").val(settingObj.autoRenameScope || "vr_only");
        $("#autoRenameScopeRow").toggle(!!settingObj.enable115AutoRename && settingObj.enable115AutoRename === YES);
        $("#enable115AutoRename").on("change", (async event => {
            const enable115AutoRename = $("#enable115AutoRename").is(":checked") ? YES : NO;
            await storageManager.saveSettingItem("enable115AutoRename", enable115AutoRename);
            $("#autoRenameScopeRow").toggle(enable115AutoRename === YES);
        }));
            ...
            if (index >= 0 && index < markedList.length) {
                ...
                utils.q(null, `确认从标记列表中移除 "${item.title}"？`, (async () => {
                    ...
                    $(`.markDeleteSvg`).each((function () {
                        ...
                        const {carNum: carNum2} = self.getBoxCarInfo($box2);
