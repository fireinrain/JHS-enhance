...
import {JavPackSubtitle} from './subtitles.js';
...
const _WangPan115MatchPlugin = class _WangPan115MatchPlugin extends BasePlugin {
    ...
    isRateLimited(response) {
        ...
        if (response.state === false) {
            ...
            if (msg.includes("频繁") || msg.includes("操作太频繁") || msg.includes("稍后再试")
                || msg.includes("rate") || msg.includes("limit") || msg.includes("too many")) {
                return true;
            }
        }
...
const extractNumberPart = (carNum) => {
    let cleaned = carNum.replace(/^fc2[-_]?(ppv[-_])?/i, "");
